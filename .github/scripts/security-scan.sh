...
for f in ${ALL_CODE_FILES[@]+"${ALL_CODE_FILES[@]}"}; do
  [[ -z "$f" ]] && continue
  if grep -nE '\beval\s*\(' "$f" 2>/dev/null | grep -v '^\s*//' | grep -v '^\s*\*' | head -5 | while IFS= read -r line; do
    block "$f" "eval() detected — code injection risk: $line"
  done; then :; fi

  if grep -nE '\bnew\s+Function\s*\(' "$f" 2>/dev/null | grep -v '^\s*//' | grep -v '^\s*\*' | head -5 | while IFS= read -r line; do
...
for f in ${ALL_CODE_FILES[@]+"${ALL_CODE_FILES[@]}"}; do
  ...
  if grep -nE 'require\s*\([^)]*\+' "$f" 2>/dev/null | grep -v '^\s*//' | head -5 | while IFS= read -r line; do
...
for f in ${ALL_CODE_FILES[@]+"${ALL_CODE_FILES[@]}"}; do
  [[ -z "$f" ]] && continue
  if grep -nE '\.innerHTML\s*=' "$f" 2>/dev/null | grep -v '^\s*//' | head -5 | while IFS= read -r line; do
...
for f in ${ALL_CODE_FILES[@]+"${ALL_CODE_FILES[@]}"}; do
  ...
  for pattern in "${SECRET_PATTERNS[@]}"; do
    if grep -nE "$pattern" "$f" 2>/dev/null | grep -v '^\s*//' | head -3 | while IFS= read -r line; do
...
for f in ${XML_FILES[@]+"${XML_FILES[@]}"}; do
  ...
  if grep -nE '<password>[^<]{20,}</password>' "$f" 2>/dev/null | grep -vi 'YOUR_\|PLACEHOLDER\|CHANGEME\|TODO\|xxx' | head -3 | while IFS= read -r line; do
...
for f in ${ALL_CODE_FILES[@]+"${ALL_CODE_FILES[@]}"}; do
  [[ -z "$f" ]] && continue
  if grep -nE 'Math\.random\s*\(' "$f" 2>/dev/null | grep -v '^\s*//' | head -3 | while IFS= read -r line; do
...
for f in ${JS_FILES[@]+"${JS_FILES[@]}"}; do
  [[ -z "$f" ]] && continue
  if grep -nE 'setRequestHeader\s*\(\s*['\''"]Authorization' "$f" 2>/dev/null | head -3 | while IFS= read -r line; do
...
for f in ${JS_FILES[@]+"${JS_FILES[@]}"}; do
  [[ -z "$f" ]] && continue
  if grep -nE 'console\.(log|debug|info|warn|error)\s*\(' "$f" 2>/dev/null | grep -v '^\s*//' | head -5 | while IFS= read -r line; do
...
for f in ${XML_FILES[@]+"${XML_FILES[@]}"}; do
  [[ -z "$f" ]] && continue
  if grep -nE 'site-id="[^"]*"' "$f" 2>/dev/null | grep -v 'SITEID' | grep -v 'mode="delete"' | head -3 | while IFS= read -r line; do
...
for f in ${ALL_CODE_FILES[@]+"${ALL_CODE_FILES[@]}"}; do
  [[ -z "$f" ]] && continue
  if grep -nE "['\"]/usr/|['\"]/tmp/|['\"]/home/|['\"]/var/|['\"]C:\\\\" "$f" 2>/dev/null | grep -v '^\s*//' | head -3 | while IFS= read -r line; do
